...
class DatabaseInstancesClientJSON(rest_client.RestClient):

    def __init__(self, auth_provider):
        super(DatabaseInstancesClientJSON, self).__init__(auth_provider)
    ...
    def get_instance_details(self, instance_id):
        """Get instance details for referenced id."""
        resp, body = self.get("instances/%s" % instance_id)
        return resp, self._parse_resp(body)

    def create_instance(self, name, flavor_ref, **kwargs):
        """
        Creates a Trove instance.
        name (Required): The name of the instance.
        flavor_ref (Required): The flavor used to build the server.
        Following optional keyword arguments are accepted:
        volume: Creates a Cinder volume of the specified size for data.
        databases: List of database 'name' dicts.
        users: List of user 'name', 'password' and 'databases' dicts. The
        databases dict is the same as databases: above.
        restorePoint: Dict of 'backupRef' to create instance from.
        availability_zone: Availability zone in which to launch instance.
        datastore: Dict of 'type' and 'version' of datastore to use.
        nics: Dict of 'net-id' or 'port-id' ('v4-fixed-ip' optional).
        configuration: JSON representation of configuration options.
        replica_of: ID of instance to set as a replication source.
        """
        post_body = {
            'name': name,
        ...
        for option in ['volume', 'databases', 'users', 'restorePoint'
                       'availability_zone', 'datastore',
                       'nics', 'configuration', 'replica_of']:
        ...
        resp, body = self.post('instances', post_body)
